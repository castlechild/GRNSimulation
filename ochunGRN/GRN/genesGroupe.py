...
def subgraph3N(Graph):
    ...
    for i, (u, v, w) in enumerate(combinations(nodes, 3)):
        if i % 1000000 == 0:
            print(f"""Queued {i}/{total_combinations} combinations
                  ({i/total_combinations*100:.2f}%)""")
...
def subgraph3N_parallel(Graph):
    ...
    def process_combination(combo):
        ...
        if issubgraphconnected(Graph, nodes[u], nodes[v], nodes[w]):
            Subgraph = nx.subgraph_view(Graph, filter_node=lambda n:
                                        n in [nodes[u], nodes[v], nodes[w]])
    ...
    with ProcessPoolExecutor(max_workers=multiprocessing.cpu_count()
        ...
        for i, result in enumerate(
            executor.map(process_combination,
                         combinations(range(len(nodes)), 3))):
            if i % 50000 == 0:
                print(i)
            if result:
                key, value = result
