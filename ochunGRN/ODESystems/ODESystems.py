...
def simulationODEs(GenesDict: dict, ODEs: list, T: tuple, Coeff: dict = None):
    ...
    Parameters:
    - GenesDict (dict): A dictionary containing information about the genes,
    including the adjacency matrix and the number of genes.
    - ODEs (list): A list of strings indicating the types of ODEs to simulate
    (e.g., "massAction", "Hill", "indirect").
    - T (tuple): A tuple of two values indicating the start and end of
    the simulation (t0, tf).
    - Coeff (dict, optional): A dictionary containing the coefficients for
    the simulation. If no coefficients are provided, they will be generated
    automatically.
